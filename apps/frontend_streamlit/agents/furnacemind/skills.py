"""SkillEngine â€” pre-computes analysis and builds operator prompts.
...
from apps.frontend_streamlit.agents.furnacemind.prompts import HEATLOAD_PLOT_CODE, HEATLOAD_REPORT_TEMPLATE
from apps.frontend_streamlit.agents.furnace_tools import fetch_ml_data, load_static_shift_data
...
def _load_params() -> dict:
    ...
    raise FileNotFoundError(
        f"FurnaceMind skill parameters not found in packaged assets: {_PARAMS_PATH}"
    )


# Load once at module import â€” YAML is ~2 KB, negligible cost.
_PARAMS: dict = _load_params()


# â”€â”€ Low-level helpers (also useful for testing in isolation) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ SkillEngine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class SkillEngine:
    ...
    # â”€â”€ Public skill entry points â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def optimise_prompt(self) -> str:
        ...
        self._build_gap_figure(
            data["tier1"], title="Unit Cost Optimisation â€” Tier 1 Gap"
        )
    ...
    def shift_to_best_prompt(self, shift_date: str, label: str) -> str:
        ...
        if shift_df is None or shift_df.empty:
            return (
                f"SKILL: Shift to Best â€” {shift_date} Shift {label}\n"
        ...
        self._build_gap_figure(
            tier1_rows, title=f"Shift to Best â€” {shift_date} Shift {label}"
        )

        uc = col_mean(shift_df, self._uc_col)
        uc_benchmark = self._fetch_uc_benchmark()
        # Restore shift df â€” benchmark fetch overwrites session_state["fm_df"]
        ...
        return (
            ...
            "Write the operator report ONLY â€” no code, no tool calls needed.\n\n"
            ...
            "  1. [param]: [current] â†’ [target] | ACTION: [specific setpoint] | REASON: [from SKILLS_BESTSHIFT] | MAGNITUDE: [effect]\n"
            "  2. ...\n"
            "  3. ...\n"
    ...
    def heatload_prompt(self) -> str:
        ...
        return (
            ...
            "STEP 1 â€” Call: fetch_online_data(lookback_hours=8, "
            'measurement_groups=["heatload_delta_t","temperature_profile","process_params"])\n\n'
            "STEP 2 â€” Call: execute_python_plot with this exact code (copy verbatim):\n"
            f"{HEATLOAD_PLOT_CODE}\n\n"
            "STEP 3 â€” After both tool calls, write this report (fill in actual numbers from the data):\n"
    ...
    def shift_report_prompt(
        ...
        """
        return (
            f"SKILL: Shift Handover Report â€” {shift_date} Shift {label}\n"
            "Do NOT call execute_python_plot â€” this skill produces text tables only, no charts. "
            ...
            "  HM Si/S/Fe           â†’ 'Offline[HM_Slag] - chem_pct_si/s/fe'\n"
            "  Slag basicity inputs â†’ 'Offline[HM_Slag] - slag_pct_cao', 'Offline[HM_Slag] - slag_pct_sio2'\n"
            "  Coke                 â†’ SUM 'Offline[Charge] - coke_1_mt' through 'coke_2_mt'\n"
            "  Nut coke             â†’ SUM 'Offline[Charge] - nut_coke_1_mt' through 'nut_coke_2_mt'\n"
            "  Sinter               â†’ SUM 'Offline[Charge] - sinter_1_mt' through 'sinter_4_mt'\n"
            "  Ore                  â†’ SUM 'Offline[Charge] - ore_1_mt' through 'ore_12_mt'\n"
            "  Pellet               â†’ SUM 'Offline[Charge] - pellet_1_mt' through 'pellet_2_mt'\n"
            "  Flux                 â†’ SUM 'Offline[Charge] - flux_1_mt' through 'flux_3_mt'\n"
            "  PCI                  â†’ 'Offline[Charge] - pci_mt'  (SUM)\n"
            ...
            "--- STEPS ---\n"
            f"STEP 1 â€” Call fetch_online_data with:\n"
            ...
            f"STEP 2 â€” Call fetch_offline_data with:\n"
            f"  report_type='HM_SLAG'\n"
            f"  start_time_utc='{start_utc}'\n"
            f"  end_time_utc='{end_utc}'\n\n"
            f"STEP 3 â€” Call fetch_offline_data with:\n"
            f"  report_type='CHARGE'\n"
            f"  start_time_utc='{start_utc}'\n"
            f"  end_time_utc='{end_utc}'\n\n"
            "STEP 4 â€” Using all fetched data, write the SHIFT HANDOVER REPORT "
            "following the template in SKILLS_SHIFTREPORT.md exactly.\n\n"
            f"Shift window: {shift_date} Shift {label}  |  UTC: {start_utc} â†’ {end_utc}\n"
        )

    # â”€â”€ Data computation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _compute_optimise_data(self) -> tuple[dict, pd.DataFrame | None]:
        ...
        # 4h-lagged window for burden angle params: rows 8â€“12 hours ago
    ...
    def _compute_tier2_dict(self, df: pd.DataFrame) -> dict[str, dict]:
        ...
        for col, cfg in self._t2.items():
            ...
            result[col] = {
                "value": v,
                "band": f"{lo}â€“{hi}" if lo is not None else "â€”",
                "flag": flag,
            }
        return result
    ...
    # â”€â”€ Figure builders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Prompt formatters â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _format_optimise_prompt(self, data: dict) -> str:
        ...
        return (
            ...
            "Write the operator report ONLY â€” no code, no tool calls.\n\n"
            ...
            f"TIER 1 PARAMETERS (ranked by impact score = gap Ã— |coefficient|):\n{t1}\n\n"
            ...
            "**Guardrails**: [CLEAR or WARNING â€” name the param]\n"
            "**Top 3 actions** (adverse=YES first, then by impact_score):\n"
            "  1. [param]: [current] â†’ [target] | ACTION: [specific setpoint] | REASON: [from SKILLS_BESTSHIFT.md] | MAGNITUDE: [expected effect]\n"
            "  2. ...\n"
            "  3. ...\n"
    ...
    def _optimise_fallback(error: str) -> str:
        ...
        return (
            f"SKILL: Optimise Unit Cost â€” pre-computation failed: {error}. "
