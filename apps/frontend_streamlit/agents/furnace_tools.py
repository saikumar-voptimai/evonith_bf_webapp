...
1. ``fetch_online_data`` â€” fetch InfluxDB telemetry for any measurement group.
2. ``fetch_offline_data`` â€” fetch shift/daily report data from the offline database.
3. ``merge_furnace_data`` â€” align and merge online + offline datasets on timestamps.
4. ``fetch_ml_data`` â€” load a date-range slice from the static pre-merged ML dataset.
5. ``concat_datasets`` â€” concatenate multiple datasets vertically (temporal union).
6. ``load_static_shift_data`` â€” load 8-hour shift data from the static ML dataset.
7. ``search_shift_history`` â€” semantic vector search over Qdrant shift summaries.
8. ``search_knowledge_docs`` â€” semantic search over uploaded operator documents.
9. ``execute_python_plot`` â€” sandboxed execution of agent-generated Plotly code.
"""
...
from apps.frontend_streamlit.data.fetch_presets import (
...
from apps.frontend_streamlit.utils.shift_windows import shift_window_naive
...
class OnlineFetchArgs(BaseModel):
    lookback: Optional[str] = Field(
        ...
        description=(
            ...
            "Use this OR start_time_utc/end_time_utc â€” never both."
        ),
    )
    ...
    *,
    ...
    try:
        ...
        if overlap_start > overlap_end + pd.Timedelta(hours=1):
            return (
                f"Requested range ({req_start} â€“ {req_end} IST) has no overlap with the "
                f"static ML dataset ({csv_start} â€“ {csv_end} IST). "
        ...
        summary = (
            ...
            f"dataset_id={dataset_id} | {len(slice_df)} rows Ã— {len(slice_df.columns)} cols\n"
            f"Columns available:\n{col_summary}"
        )
        ...
        if req_end > csv_end + gap_threshold:
            ...
            summary += (
                ...
                f"Note: online columns use InfluxDB names (e.g. 'fuel_rate') â€” ML static uses ML names (e.g. 'ACT. FUEL RATEKG/THM.'). "
...
def concat_datasets(*, dataset_ids: list[str]) -> str:
    ...
    Duplicate timestamps keep the last entry (later dataset wins â€” prefer online for recent rows).
    ...
    try:
            ...
            for f in frames:
                if isinstance(f.index, pd.DatetimeIndex):
                    if f.index.tz is None:
                        # IST-naive â†’ localise to IST then convert to UTC
                        f = f.copy()
        ...
        return (
            ...
            f"dataset_id={dataset_id} | {len(combined)} rows Ã— {len(combined.columns)} cols\n"
...
def fetch_online_data(
    *,
    ...
    # Legacy params â€” accepted but ignored to avoid errors if LLM still sends them
    ...
    ``start_time_utc`` + ``end_time_utc`` for an exact window â€” never both.
    ...
    try:
        ...
        if args.start_time_utc:
            ...
            time_range_label = f"{args.start_time_utc} â†’ {args.end_time_utc or 'now'}"
        else:
            # Relative lookback path â€” normalise the string to a TIMEDELTAS key
    ...
    return [
        {
            ...
            "function": {
                ...
                "description": (
                    ...
                    "OR start_time_utc + end_time_utc for exact windows â€” never both. "
        ...
        },
        {
            ...
            "function": {
                ...
                "description": (
                    ...
                    "Fast â€” no InfluxDB call. Covers process params, material quality, burden, KPIs, hot metal chemistry. "
                    ...
                    "Use fetch_online_data directly only for: last â‰¤2 days, sub-hourly resolution, or when this tool reports no coverage."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
        ...
        },
        {
            ...
            "function": {
                ...
                "description": (
                    "Load 8-hour shift data from the static ML dataset (hourly, online+offline pre-merged). "
                    "Covers Jan 2024 to Mar 2026. Returns error if the shift is outside this range â€” "
...
def execute_python_plot(code: str) -> str:
    ...
    PRE-LOADED â€” do NOT import these, they are already available:
      pd            â€” pandas
      px            â€” plotly.express
      go            â€” plotly.graph_objects
      make_subplots â€” plotly.subplots.make_subplots
      np            â€” numpy
      df            â€” current DataFrame (most recently fetched dataset)

    RULES â€” the sandbox will reject code that violates these:
      - DO NOT use 'import', '__import__', 'open(', 'os', 'subprocess', 'sys', 'eval', 'exec'
      - DO NOT call fig.show() â€” the figure is rendered automatically by the UI
      - The code MUST assign a Plotly figure to a variable named 'fig'

    DIAGNOSTIC USE: Code that only calls print() (no fig) is allowed â€” the output
    ...
    try:
        ...
        import numpy as np  # noqa: PLC0415 â€” local import intentional for sandbox context
        ...
        if "fig" in local_vars:
            ...
            # Diagnostic code (e.g. print(df.columns)) â€” return output so the LLM
