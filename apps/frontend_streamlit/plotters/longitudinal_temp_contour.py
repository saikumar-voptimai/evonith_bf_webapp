...
geometry profile.  Also provides a Plotly-compatible rendering path via
:meth:`LongitudinalTemperaturePlotter.plot_plotly`.
"""
...
class LongitudinalTemperaturePlotter(BasePlotter):
    """Longitudinal cross-section temperature plotter.

    Renders per-quadrant vertical temperature distributions on the furnace
    geometry, including zone (region) labels and a shared colorbar.
    ...
    def plot(self, temperatures_list):
        ...
        global_max = (
            int(max(max(values) for values in temperatures_list) / step) * step + step
        )
        ...
        for idx, (temperatures, ax) in enumerate(zip(temperatures_list, axes)):
            ...
            contour = ax.contourf(
                X, Y, Z, levels=100, cmap="viridis", vmin=global_min, vmax=global_max
            )
            ...
            boundary_x = (
                [-4] + [p[0] for p in self.furnace.geometry_points] + [0, 0, -4]
            )
        ...
        Z_hid[Z_hid < global_max + 0.5] = global_min
        vmin, vmax = global_min, global_max
        hidden_contour = hidden_ax.contourf(
            X, Y, Z_hid, levels=100, cmap="viridis", vmin=vmin, vmax=vmax
        )
        hidden_ax.remove()
    ...
    def plot_plotly(
        self, temperatures_list, temperatures_max_list, temperatures_min_list
    ):
        ...
        fig = make_subplots(
            ...
            column_widths=[0.3, 0.22, 0.22, 0.22],
        )
        # Iterate over each quadrant's temperatures
        for idx, (temperatures, temperatures_max, temperatures_min) in enumerate(
            zip(temperatures_list, temperatures_max_list, temperatures_min_list)
        ):
            ...
            fig.add_trace(
                go.Heatmap(
                    ...
                    colorbar=(
                        ...
                        if idx == len(temperatures_list) - 1
                        else None
                    ),
        ...
        fig.update_layout(
            height=400, margin=dict(t=40, b=20, l=20, r=20), plot_bgcolor="white"
        )
        return fig
