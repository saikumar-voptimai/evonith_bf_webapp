...
from furnace_data.optimization_runtime import DatasetContextService, build_runtime_config
from furnace_data.bmo.utils.types import FluxInput, OreChemistry, OreInput
...
def _resolve_repo_path(path_str: str) -> Path:
    """Resolve BMO config paths against packaged config, repo root, and cwd."""
    p = Path(path_str)
    if p.is_absolute():
        return p

    config_candidate = get_config_path(p.name)
    repo_root = get_repo_root()
    candidates = [config_candidate, repo_root / p, Path.cwd() / p]
...
class EvonithBmoContextProvider:
    ...
    def __init__(
        self,
        setting_path: str = "setting_bmo.yml",
        mapping_path: str = "bmo_ore_mapping.yml",
    ) -> None:
        """
    ...
    def _dataset_rename_dict(self) -> dict[str, str]:
        config = _load_yaml(get_config_path("setting_ds_dv.yml"))
    ...
    def _charge_columns_by_group(self) -> dict[str, list[str]]:
        config = _load_yaml(get_config_path("shift_report.yml"))
