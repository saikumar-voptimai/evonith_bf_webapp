"""
Tests for StaticDatasetManager — the lag-aware cache logic.
...
from app.core.static_manager import (
    CacheMeta,
...
class TestCacheMeta:
    ...
    def test_round_trips_json(self, tmp_path):
        meta = CacheMeta(
            ...
            rows=1000,
            columns=50,
            csv_file="ml_dataset_20260401.csv",
        )
...
# ---------------------------------------------------------------------------
# save() — CSV written and meta updated
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# update_static() — end-to-end with mocked fetcher
# ---------------------------------------------------------------------------

class TestUpdateStatic:
    def _make_manager(self, tmp_path, legacy=None):
        with patch("app.core.static_manager.MlDatasetFetcher") as MockFetcher:
