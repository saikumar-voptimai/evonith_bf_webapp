...
def test_furnacemind_config_conversation_run_events_documents_tools_feedback_and_openapi(app_factory, monkeypatch, tmp_path):
    with _client(
        ...
        msg = client.post(
            f"/api/v1/furnacemind/conversations/{conversation_id}/messages",
            json={"content": "hello"},
        )
        upload = client.post(
            "/api/v1/furnacemind/documents",
            files={"file": ("sop.txt", b"pressure stability", "text/plain")},
        )
        document_id = upload.json()["data"]["id"]
        indexed = client.post(f"/api/v1/furnacemind/documents/{document_id}/index")
        ...
        run = client.post(
            ...
            json={
                ...
                "allow_llm": True,
                "options": {
                    "tool_calls": [{"name": "data_summary", "input": {"rows": [{"x": 1}]}}],
                    "export": True,
                },
            },
        )
...
def test_furnacemind_security_errors_and_disabled_modes(app_factory, monkeypatch, tmp_path):
    ...
    with _client(app_factory, monkeypatch, tmp_path) as client:
        ...
        index_disabled = client.post(f"/api/v1/furnacemind/documents/{pdf_id}/index")
        tool_disabled = client.post(
            f"/api/v1/furnacemind/conversations/{conversation_id}/runs",
    ...
    assert index_disabled.status_code == 200
    assert index_disabled.json()["data"]["warnings"][0]["code"] == "FURNACEMIND_MEMORY_DISABLED"
    assert tool_disabled.status_code == 200
    result_message = tool_disabled.json()["data"]["assistant_message_id"]
