...
def query_builder(measurement: str, start: str, stop: str, type: str='average', window_by: str='1h') -> str:
    """
    ...
    if type == 'ts':
        ...
        raise ValueError(f"Invalid query type: {type}. Use 'ts', 'average' or 'avg-min-max'.")
...
class BaseDataFetcher:
    ...
    def fetch_averaged_data(
        ...
        query = query_builder(self.measurement_type, 
                              start_time, 
                              end_time, 
                              type=request_type, 
                              window_by=window_by)
