...
All calibration numbers live in ``data/copilot/skill_params.yml``.
...
class SkillEngine:
    ...
    stored in ``st.session_state["copilot_fig"]`` before the LLM is called.
    """
    ...
    def shift_to_best_prompt(self, shift_date: str, label: str) -> str:
        ...
        shift_df: pd.DataFrame | None = st.session_state.get("copilot_df")
    ...
    def _compute_optimise_data(self) -> tuple[dict, pd.DataFrame | None]:
        ...
        df: pd.DataFrame | None = st.session_state.get("copilot_df")
    ...
    def _build_gap_figure(self, tier1_rows: list[dict], *, title: str) -> None:
        ...
        st.session_state["copilot_fig"] = fig
