"""Scheduled Tasks workspace for creating and managing operator-owned jobs.

The page builds validated portable definitions, stores them through the
persistence service, and exposes owner-scoped job details and result history.
Lifecycle and edit requests are written to a PostgreSQL command queue for the
target Jetson; the Streamlit process never manipulates systemd directly.
"""
...
@st.cache_resource(show_spinner=False)
def _get_scheduled_job_command_service() -> ScheduledJobCommandService:
    """Return the shared PostgreSQL-backed lifecycle command service."""

    return ScheduledJobCommandService()
...
def _queue_task_definition_update(
    definition: dict[str, object],
    *,
    operator: str,
) -> ScheduledJobCommandView:
    """Queue a revision-safe update for the task loaded into the form."""
    ...
    return _get_scheduled_job_command_service().request_action(
        job_id=job_id,
        action="update",
        owner_user_id=user_id,
        requested_by_username=operator,
        expected_job_updated_at=expected_updated_at,
        definition=definition,
    )
...
def _render_generated_file(task: ScheduledTaskInput) -> None:
    ...
    ready_title = "Changes queued" if is_update else "Task saved"
    ready_copy = (
        "The BF2 Jetson will apply this revision shortly."
        if is_update
        else "The definition is waiting for timer setup."
    )
    st.markdown(
        f"""
...
def _management_action(status: str) -> tuple[str, str] | None:
    """Return the primary lifecycle action and label for a job status."""
...
def _execute_management_action(
    *,
    ...
    """Recheck ownership and enqueue an allow-listed target-device command."""

    service = _get_scheduled_job_service()
    try:
        owned_job = service.get_job_for_owner(job_id, owner_user_id)
        if owned_job is None:
            st.session_state[_ACTION_MESSAGE_KEY] = (
                "error",
                "This task is no longer available for this operator.",
            )
            return
        command = _get_scheduled_job_command_service().request_action(
            job_id=job_id,
            action=action,
            owner_user_id=owner_user_id,
            requested_by_username=operator,
            expected_job_updated_at=owned_job.updated_at,
        )
    except (ScheduledJobPersistenceError, ScheduledJobValidationError):
        st.session_state[_ACTION_MESSAGE_KEY] = (
            "error",
            "The task could not be verified. Please refresh and try again.",
        )
    except ScheduledJobCommandConflictError:
        st.session_state[_ACTION_MESSAGE_KEY] = (
            "error",
            "The task changed or already has a pending request. Refresh and try again.",
        )
    else:
        label = {
            ...
            "resume": "Resume",
            "archive": "Archive",
        }.get(command.action, "Lifecycle")
        st.session_state[_ACTION_MESSAGE_KEY] = (
            "success",
            f"{label} request queued for the BF2 Jetson.",
        )


def _render_action_message() -> None:
    """Render and consume the latest lifecycle-action feedback."""
...
def _render_run_history(
    history: tuple[ScheduledJobRunHistoryItem, ...],
    timezone_name: str,
) -> None:
    """Render recent run outcomes and their stored result content."""

    if not history:
        st.info("No runs have been recorded for this task yet.")
        return
    for item in history:
        status = item.status.replace("_", " ").title()
        scheduled = _format_job_timestamp(item.scheduled_for, timezone_name)
        with st.expander(f"{status} · {scheduled}", expanded=False):
            st.caption(
                f"Attempt {item.attempt_number} · "
                f"Triggered by {item.triggered_by or 'timer'} · "
            ...
            if item.content_json:
                st.json(item.content_json, expanded=False)
            if not item.content and not item.content_json and not item.error_message:
                st.caption("This run has no stored output.")
...
def _render_task_management(
    ...
    """Render owner-scoped saved jobs, lifecycle controls, and run history."""
    ...
    with st.container(border=True, key="scheduled_task_manager"):
        st.markdown(
            ...
            <div class="scheduled-task-creator-heading">
                ...
                <p>Inspect saved definitions, control timers, and review recent results.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        command_status_unavailable = False
        try:
            latest_command = _get_scheduled_job_command_service().latest_for_owner(
                job_id=selected.job_id,
                owner_user_id=owner_user_id,
            )
        except ScheduledJobPersistenceError:
            latest_command = None
            command_status_unavailable = True
        command_open = latest_command is not None and latest_command.status in {
            "pending",
            "processing",
        }
        controls_disabled = command_status_unavailable or command_open
        if latest_command is not None:
            action_label = latest_command.action.replace("_", " ").title()
            if latest_command.status == "pending":
                st.info(f"{action_label} request is waiting for the BF2 Jetson.")
            elif latest_command.status == "processing":
                st.info(f"{action_label} request is being applied on the BF2 Jetson.")
            elif latest_command.status == "failed":
                st.warning(
                    f"The last {latest_command.action.replace('_', ' ')} request failed. "
                    "You can try again or ask an administrator to inspect the Jetson logs."
                )
        primary = _management_action(selected.status)
        action_col, edit_col, clone_col, archive_col = st.columns(4)
        with action_col:
        ...
        with archive_col:
            if st.button(
                ...
                width="stretch",
                disabled=controls_disabled or selected.status == "deleted",
            ):
                st.session_state[_ARCHIVE_CONFIRM_KEY] = selected.job_id

        if command_status_unavailable:
            st.caption(
                "Task controls are temporarily unavailable because the command queue could not be reached."
            )
        if st.session_state.get(_ARCHIVE_CONFIRM_KEY) == selected.job_id:
            st.warning(
                "Archive this task? Its timer will be removed, while its definition and run history remain available."
            )
        ...
        overview_tab, runs_tab, changes_tab, json_tab = st.tabs(
            ("Overview", "Run history", "Changes", "JSON")
        )
        with overview_tab:
            ...
            st.caption(f"Created {created} by {creator} · Job ID {selected.job_id}")
            if selected.provisioning_error:
                st.warning("Timer setup needs administrator attention.")
        with runs_tab:
            try:
                history = service.list_run_history(
                    selected.job_id,
                    owner_user_id,
                    limit=25,
                )
            except (ScheduledJobPersistenceError, ScheduledJobValidationError):
                st.error("Run history could not be loaded.")
            else:
                _render_run_history(history, timezone_name)
        with changes_tab:
            try:
                revisions = (
                    _get_scheduled_job_command_service().list_revisions_for_owner(
                        job_id=selected.job_id,
                        owner_user_id=owner_user_id,
                        limit=25,
                    )
                ...
                st.error("Task changes could not be loaded.")
            else:
                for revision in revisions:
                    changed = _format_job_timestamp(
                        revision.created_at,
                        timezone_name,
                    )
                    label = revision.change_kind.replace("_", " ").title()
                    st.markdown(
                        f"**Revision {revision.revision_number}** · {label} · {changed}"
                    )
                    st.caption(f"Changed by {revision.changed_by_username}")
        with json_tab:
...
def render_scheduled_tasks_page() -> None:
    ...
    with st.container(key="scheduled_task_shell"):
        ...
        heading_copy = (
            "Review the full definition. The Jetson will safely replace the timer after you save."
            if editing
            else "Define what FurnaceMind should do, when it should run, and where the result should go."
        )
        with st.container(border=True, key="scheduled_task_creator"):
            ...
            with st.container(border=False, key="scheduled_task_advanced"):
                with st.expander("Advanced", expanded=False):
                    ...
                    st.caption(
                        "Runs will not overlap. After downtime, only the latest missed run is recovered."
                    )
                ...
                if validation_errors:
                    ...
                    try:
                        ...
                        st.session_state[_BUILD_ERROR_KEY] = (
                            "The task changed or already has a pending request. "
                            "Return to Your tasks, refresh it, and try again."
                        )
                        st.session_state.pop(_GENERATED_KEY, None)
