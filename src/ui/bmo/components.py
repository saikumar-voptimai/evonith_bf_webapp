...
def _safe_dataframe(df: pd.DataFrame, *, hide_index: bool = True, use_container_width: bool = True) -> None:
...
def render_header(bundle_status: dict[str, Any]) -> None:
    ...
    status_class = "bmo-status-ok" if bundle_status.get("model_loaded") else "bmo-status-warn"

    st.markdown(
        f"""
        <div class="bmo-header">
          <h2>Blend Mix Optimizer (BMO)</h2>
          <p>Ticket 104 integration: LP baseline + nonlinear total-cost optimization inside Evonith.</p>
        </div>
...
def build_ore_editor_df(ores: list[OreInput], default_selected_ids: list[str]) -> pd.DataFrame:
...
def render_ore_editor(editor_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return editor_fn(
        ...
        column_config={
            ...
            "stock_mt": st.column_config.NumberColumn("Stock (MT)", min_value=0.0, step=10.0),
...
def render_blend_table(blend: BlendEvaluation, selected_ores: list[OreInput]) -> None:
    ...
    if hasattr(st, "column_config"):
        st.dataframe(
            df,
            hide_index=True if "hide_index" in inspect.signature(st.dataframe).parameters else False,
            use_container_width=True
            ...
            column_config={
                ...
                "price_rs_per_mt": st.column_config.NumberColumn("Price (Rs/MT)", format="%.1f"),
                "ore_cost_rs": st.column_config.NumberColumn("Ore Cost (Rs)", format="%.0f"),
            },
        )
    else:
...
def render_diagnostics(blend: BlendEvaluation | None, diagnostics: dict[str, Any]) -> None:
