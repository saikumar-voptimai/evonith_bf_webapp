"""Channeling and instability propensity scoring for the BF2 furnace.

Contains two public classes:

* :class:`ChannelingConfig` — dataclass that holds all threshold and column-
  mapping parameters for the detector.
* :class:`Channeling` — computes a 0–1 channeling score for each rolling
  10-minute window from raw sensor DataFrames.

Typical usage::

    cfg = ChannelingConfig(window="10min", step="10min")
    detector = Channeling(cfg)
    score_df = detector.score_timeseries(df)
"""
...
class ChannelingConfig:
    """Configuration for the :class:`Channeling` detector.

    All thresholds normalise the raw sensor readings to a 0–1 score.
    Columns map the generic signal names to the actual DataFrame column
    labels present in the online InfluxDB export.

    Attributes:
        window:                      Rolling window size (Pandas timedelta string).
        step:                        Step size between successive windows.
        spread_mode:                 Spread algorithm — ``"max-min"`` or
                                     ``"second_max-min"`` (robust to a single spike).
        max_spread_uptake_temp:      Maximum expected uptake temperature spread (°C).
        max_spread_uptake_pressure:  Maximum expected uptake pressure spread (bar).
        max_spread_skin:             Maximum expected 18660mm skin temperature spread (°C).
        max_drop_hbp:                Max expected hot-blast pressure drop (bar, negative).
        max_raise_topdp:             Max expected top differential pressure rise (bar).
        max_raise_bottomdp:          Max expected bottom differential pressure rise (bar).
        max_drop_eta_co:             Max expected ETA CO drop (percentage-points, negative).
        max_rise_heatload:           Max expected heat-load rise (GJ/hr).
        uptake_pressure_cols:        Column names for the four uptake pressure signals.
        uptake_temp_cols:            Column names for the four uptake temperature signals.
        skin_18660_cols:             Column names for the four stack (18660mm) sensors.
        hbp_col:                     Column name for hot-blast pressure.
        topdp_col:                   Column name for top differential pressure.
        bottomdp_col:                Column name for bottom differential pressure.
        eta_co_col:                  Column name for ETA CO.
        heatload_cols:               Column names for the four quadrant heat-load signals.
    """
...
class Channeling:
    """Rolling-window channeling propensity scorer.

    Scores each *step*-sized time window using seven independent diagnostic
    signals: uptake temperature/pressure spread, stack skin temperature spread,
    hot-blast pressure drop, top/bottom differential pressure rise, ETA CO drop,
    and heat-load rise.  Each signal is clipped to [0, 1]; the final
    ``channeling_score`` is their equal-weight mean.

    Attributes:
        cfg: Active :class:`ChannelingConfig` instance.
    """

    def __init__(self, cfg: Optional[ChannelingConfig] = None) -> None:
        """Initialise the detector with an optional config.

        Args:
            cfg: Detector configuration.  Defaults to :class:`ChannelingConfig`
                 with all factory values if not supplied.
        """
    ...
    def _normalize_config_strings(self) -> None:
        ...
        self.cfg.uptake_pressure_cols = [
            c.strip() for c in self.cfg.uptake_pressure_cols
        ]
    ...
    def _ensure_datetime_index(
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            raise ValueError(
                "DataFrame must have a DatetimeIndex or provide time_col='your_time_column'"
            )
        return df
    ...
    def _diagnosis_uptake(self, g: pd.DataFrame) -> Dict[str, Any]:
        ...
        temp_means = (
            g[temp_cols].mean(axis=0, skipna=True).to_numpy()
            if temp_cols
            else np.array([])
        )
        pres_means = (
            g[pres_cols].mean(axis=0, skipna=True).to_numpy()
            if pres_cols
            else np.array([])
        )
        ...
        diag_temp = (
            spread_temp / self.cfg.max_spread_uptake_temp
            if np.isfinite(spread_temp)
            else np.nan
        )
        diag_pres = (
            spread_pres / self.cfg.max_spread_uptake_pressure
            if np.isfinite(spread_pres)
            else np.nan
        )

        # Treat uptake as one “block” score (mean of temp/pressure if both exist)
        uptake_score = (
            float(np.nanmean([diag_temp, diag_pres]))
            if np.isfinite(np.nanmean([diag_temp, diag_pres]))
            else np.nan
        )
    ...
    def _diagnosis_skin(self, g: pd.DataFrame) -> Dict[str, Any]:
        ...
        means_18660 = (
            g[cols_18660].mean(axis=0, skipna=True)
            if cols_18660
        ...
        skin_score = (
            spread_skin / self.cfg.max_spread_skin
            if np.isfinite(spread_skin)
            else np.nan
        )
    ...
    def _diagnosis_chg(
        self, g: pd.DataFrame, col: str, max_chg_param: float
    ) -> Dict[str, Any]:
        """
        ...
        if np.sign(delta) != np.sign(max_chg_param):  # Opposite signs => no impact
            ...
            diag = (
                (delta / max_chg_param) if max_chg_param != 0 else np.nan
            )  # (-)/(-) => positive

        return {"diag": float(diag), "delta": float(delta)}

    # ---------- (e) / (f) Sum of normalized positive rises (heatload) ----------
    def _diagnosis_rise_sum(
        self, g: pd.DataFrame, cols: List[str], max_rise_param: float
    ) -> Dict[str, Any]:
        """
            ...
            if np.isfinite(mu) and self.cfg.max_spread_uptake_temp > 0:
                for i in range(N_Q):
                    if np.isfinite(vals[i]):
                        q_contribs[i].append(
                            abs(vals[i] - mu) / self.cfg.max_spread_uptake_temp
                        )
        ...
        if len(pres_cols) == N_Q:
            ...
            if np.isfinite(mu) and self.cfg.max_spread_uptake_pressure > 0:
                for i in range(N_Q):
                    if np.isfinite(vals[i]):
                        q_contribs[i].append(
                            abs(vals[i] - mu) / self.cfg.max_spread_uptake_pressure
                        )
        ...
        if len(skin_cols) == N_Q:
            ...
            if np.isfinite(mu) and self.cfg.max_spread_skin > 0:
                for i in range(N_Q):
                    if np.isfinite(vals[i]):
                        q_contribs[i].append(
                            abs(vals[i] - mu) / self.cfg.max_spread_skin
                        )
        ...
        if len(present_hl) == N_Q:
            for i, c in enumerate(hl_cols):
                ...
                if (
                    ...
                    and self.cfg.max_rise_heatload > 0
                ):
    ...
    def _score_window(
        self, g: pd.DataFrame, window_end: pd.Timestamp
    ) -> Dict[str, Any]:
        """Score a single rolling window across all seven diagnostic signals.

        Runs each sub-diagnosis (uptake, skin, HBP drop, top/bottom DP rise,
        ETA CO drop, heat-load rise, per-quadrant profile) and aggregates to a
        single ``channeling_score`` using equal weighting across available
        components.

        Args:
            g:          Subset DataFrame covering the current rolling window.
            window_end: Timestamp label for the end of this window.

        Returns:
            Dict containing ``window_end``, all per-signal scores and deltas,
            ``q1_score``–``q4_score``, ``n_components_used``, and
            ``channeling_score``.
        """
        ...
        # (e)
        bottomdp = self._diagnosis_chg(
            g, self.cfg.bottomdp_col, self.cfg.max_raise_bottomdp
        )
        ...
        # (g)
        heat = self._diagnosis_rise_sum(
            g, self.cfg.heatload_cols, self.cfg.max_rise_heatload
        )
        ...
        out["channeling_score"] = (
            float(np.nanmean(components)) if n_used > 0 else np.nan
        )
