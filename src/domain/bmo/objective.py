...
class BmoObjectiveEvaluator:
    def __init__(
        ...
        *,
        ...
        self.min_shares = np.array([float(ore.min_share_pct) / 100.0 for ore in ores], dtype=float)
        self.max_shares = np.array([float(ore.max_share_pct) / 100.0 for ore in ores], dtype=float)
    ...
    def evaluate_shares(self, shares: np.ndarray) -> ObjectiveResult:
        ...
        if blend.fe_production_mt < self.min_fe_production_mt:
            fe_penalty += (self.min_fe_production_mt - blend.fe_production_mt) * penalty_fe
        if blend.fe_production_mt > self.max_fe_production_mt:
            fe_penalty += (blend.fe_production_mt - self.max_fe_production_mt) * penalty_fe
