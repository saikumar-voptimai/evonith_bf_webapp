...
from domain.optimization_runtime.types import OptimizationContext
...
class DatasetContextService:
    ...
    def maybe_refresh(self, config: dict[str, Any] | None = None) -> bool:
        if not self.refresh_enabled or config is None:
            return False
        try:
            from utils.dataset_refresher import maybe_refresh

            return bool(maybe_refresh(config, rm_choice=self.refresh_rm_choice))
        except Exception:
            return False
    ...
    def build_context(
        ...
        *,
        ...
        latest_row = history_df.iloc[-1].copy() if not history_df.empty else pd.Series(dtype=float)
        context = {
            str(key): float(value)
