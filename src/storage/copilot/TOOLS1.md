...
- After concat: ML columns use ML names (`ACT. FUEL RATEKG/THM.`), online uses InfluxDB names (`fuel_rate`). Plot whichever is non-null per time region.
...
- Code MUST define a variable `fig`. Use `df`, `pd`, `px`, `go` — no imports needed.
